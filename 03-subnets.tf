...
# Tokyo Syslog Private
resource "aws_subnet" "tokyo-syslog-private-subnet-1c"{                
    vpc_id = aws_vpc.tokyo-syslog-vpc.id                     
    cidr_block = var.subnet_cidr_block_tokyo-syslog-private1c             
    availability_zone = var.avail_zone_tokyo-syslog-private1c      
    provider    = aws.tokyo-syslog
    tags = {                                           
        Name: "${var.env_prefix_tokyo-syslog}-subnet-1c"     
        Service: "Armageddon Phase 1"
        Owner = "MATTRESS AVENGERS"         
    }
}